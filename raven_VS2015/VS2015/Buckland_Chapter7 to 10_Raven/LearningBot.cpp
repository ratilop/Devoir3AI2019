...
bool LearningBot::AddHumanData(vector<double> &data, vector<double>&target) {
	CData m_TrainingSet = CData();
	if (isPossessed()) {
		if (data.size() > 0 && target.size() > 0) {
			...
			if (data.size() == m_TrainingSet.GetInputNb() && target.size() == m_TrainingSet.GetTargetsNb()) {
